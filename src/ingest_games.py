...
def ensure_season(season_label: str) -> int:
    with engine.begin() as conn:
        conn.execute(
            text("INSERT INTO seasons (season_label) VALUES (:season_label) ON CONFLICT (season_label) DO NOTHING"),
...
def load_team_games_and_player_logs(team_abbreviation: str = "NYK", season: str = "2024-25") -> None:
    ...
    games["home_or_away"] = games["MATCHUP"].apply(lambda x: "Away" if "@" in x else "Home")
    ...
    final_games = games[[
        "game_id", "season_id", "game_date", "team_id", "opponent_team_id",
        "matchup", "home_or_away", "team_score", "opponent_score", "result"
    ]].drop_duplicates()

    with engine.begin() as conn:
        conn.execute(text("DELETE FROM player_game_stats"))
        conn.execute(text("DELETE FROM games"))
        final_games.to_sql("games", conn, if_exists="append", index=False)
    ...
    roster_players = pd.DataFrame({
        ...
        "first_name": roster["PLAYER"].apply(lambda x: str(x).split(" ", 1)[0] if " " in str(x) else str(x)),
        "last_name": roster["PLAYER"].apply(lambda x: str(x).split(" ", 1)[1] if " " in str(x) else ""),
    ...
    with engine.begin() as conn:
        existing_ids = pd.read_sql(text("SELECT player_id FROM players"), conn)["player_id"].tolist()
        missing_players = roster_players[~roster_players["player_id"].isin(existing_ids)]
    ...
    for player_id in roster["PLAYER_ID"].tolist():
        try:
            logs = playergamelog.PlayerGameLog(player_id=int(player_id), season=season).get_data_frames()[0]
            time.sleep(1)
    ...
    if stat_rows:
        ...
        with engine.begin() as conn:
            valid_player_ids = pd.read_sql(
                text("SELECT player_id FROM players"), conn
            )["player_id"].tolist()

            valid_game_ids = pd.read_sql(
                text("SELECT game_id FROM games"), conn
            ...
            stats_df.to_sql("player_game_stats", conn, if_exists="append", index=False)
